import gothamSite from '../assets/img/gotham_site.png';
import milkyway from '../assets/img/milkyway.png';
import yale from '../assets/img/yale-art-school.png';
import echo from '../assets/img/Echo.png';
import notemaker from '../assets/img/notemaker.png';
import kh_dark from '../assets/img/Dark_Theme.png';
import urbanlens from "../assets/img/urbanlens.png";

const projects = [
        {
            "id": 1,
            "title": "Urban Lens",
            "subtitle": "Website",
            "description": "This website allows users to and compare living costs across the US. It features dynamic filtering, a community board with CRUD capability, and user authentication",
            "date": "May 2026",
            "img": urbanlens,
            "alt": "Image of city gallery where cities can be sorted on the side by price, population, quality of life score, and other metrics",
            "software": "React.js + MaterialUI + Supabase",
            "skills": "Full-Stack",
            "url": "https://www.urbanlens.pro/",
        },
        {
            "id": 2,
            "title": "Milky Way Cafe",
            "subtitle": "website",
            "description": 'Responsive multi-page website built with React.js featuring client-side routing, a dynamic shopping bag, event calendar, and AI-generated imagery via MCP',
            "date": "Mar 2026",
            "img": milkyway,
            "alt": "image of fictious coffee shop design using brown and yellow and an AI-generated image of a coffee shop",
            "software": "React.js + CSS",
            "skills": "MCP + Global state management",
            "url": "https://milkyway2-one.vercel.app/",
        },
        {
            "id": 3,
            "title": "Gotham Mysteries",
            "subtitle": "Website",
            "description": "This website was created to practice web development technologies. Site features a landing page, character gallery, and a retro themed mini game created with Phaser.js",
            "date": "Apr 2024",
            "img": gothamSite,
            "alt": "image of character gallery from website",
            "software": "HTML/Bootstrap CSS/Phaser.js",
            "skills": "Front-End + Game",
            "url": "https://gotham-site.netlify.app/",
        },
        {
            "id": 4,
            "title": "Echo: Mobile App",
            "subtitle": "prototype",
            "description": "A mobile app group project created for a hackathon. Echo is a music app that prioritizes social connection. Figma features prototyping, a mobile navbar, login and sign up screen, home screen, and a currently playing display",
            "date": "Sep 2024",
            "img": echo,
            "alt": "image of Figma layout for a mobile music app. App features a dark purple color scheme and makes use of music-related icons. Design features 6 screens showcasing different views within the app",
            "software": "Figma",
            "skills": "UI Design + Prototyping",
            "url": "https://www.figma.com/file/7D4m22giostMwGkHxBL6uV/LetterBox-Spotify",

        },
        // {
        //     "id": 5,
        //     "title": "Yale School of Art",
        //     "subtitle": "Redesign",
        //     "description": "Mock-up redesign of the Yale School of Art. Design is inline with branding colors and matches the style of their other websites, which is in significant contrast to the actual website",
        //     "date": "Oct 2024",
        //     "img": yale,
        //     "alt": "image of redesign for yale school of art website",
        //     "software": "Figma",
        //     "skills": "UI Design + Web Design",
        //     "url": "https://www.figma.com/file/t2fUlWqkctfgdxpuHNn4p0/Yale-School-of-Art",
        // },
        {
            "id": 5,
            "title": "NOTEMAKER",
            "subtitle": "Mobile App Design + Code",
            "description": "To-Do tracking app created in mobile development class. Created design first using Figma, and then executed design and coded it using React Native and Expo Go",
            "date": "Feb 2025",
            "img": notemaker,
            "alt": "Image of four screen mobile app layout. Design uses a shade of pink and features a log in screen, navbar, upcoming tasks, add tasks screen, and a completed tasks display",
            "software": "Figma + Expo Go + React Native",
            "skills": "Mobile Design + Development",
            "url": "https://youtube.com/shorts/SGUAIv_gmF4?si=UExVGI0SgZDeZ7wW",
        },
        {
            "id": 6,
            "title": "KnightHacks Neon Logo",
            "subtitle": "Dark Theme",
            "description": "Graphic design project for club brand. Created a neon glow outline of club logo to be used across multiple social media profiles of the club",
            "date": "Nov 2024",
            "img": kh_dark,
            "alt": "Image of Dragon curled in a circle with a sword going through it featuring the title 'KNIGHT HACKS' to the right of it. Logo is transparent and features a neon purple glow outline around it.",
            "software": "Adobe Illustrator",
            "skills": "Graphic Design",
            "url": "https://www.linkedin.com/in/meera-bhola/overlay/Project/1359116227/treasury/?profileId=ACoAAD4U-YcBdPSxdtJn0IxG7Z5uaxQQw-FiL7I",
        }
];

export default projects;